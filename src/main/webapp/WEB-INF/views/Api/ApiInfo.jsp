	...
	<table class="join_chart">
		...
		<tbody>
			<tr>
				...
				<td>
					<input type="text" id="partnerEmail" name="partnerEmail" required/>
					<a href="#" id="btn-id-check" class="btn-sm spot fn" title="중복확인(새창열림)"><span>중복확인</span></a>
				</td>
			</tr>
			<tr>
				<th><strong class="star">*</strong><label for="partnerNm">Username</label></th>
				<td><input type="text" id="partnerNm" name="partnerNm" required/></td>
			</tr>
			<tr>
				<th><strong class="star">*</strong><label for="partnerPwd">Password</label></th>
				<td><input type="password" id="partnerPwd" name="partnerPwd" required/></td>
			</tr>
		</tbody>
	</table>
...
function regist(){
	 if($("#idCheckAt").val() != "Y"){
		...
		return false;
	}else if(!$("#partnerEmail").val()){
		alert("아이디를 입력해주세요.");
		return false;
	}else if(!$("#partnerNm").val()){
		alert("이름을 입력해주세요.");
		return false;
	}else if(!$("#partnerPwd").val()){
		alert("비밀번호를 입력해주세요.");
		return false;
